instructions = """um inc -671 if lbf != 5
j inc 236 if umr > -6
fk inc -246 if j < 241
uy dec -404 if mmw <= 2
j inc 372 if gk >= -1
...
for instruction in instructions.splitlines():
    ...
    values.setdefault(condition_parts[0], 0)
    condition_parts[0] = "values['"+condition_parts[0]+"']"
    print(action, condition_parts)
    print(condition_parts[1])
    if eval(' '.join(condition_parts)):
            action_parts = action.replace("inc", "+=").replace("dec", "-=").split()
            values.setdefault(action_parts[0], 0)
            action_parts[0] = "values['"+action_parts[0]+"']"
            exec(' '.join(action_parts))
            max_val = max(max_val, eval(action_parts[0]))
